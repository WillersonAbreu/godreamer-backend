...
{   
    public function Index($id)
    {
        ...
        $amigos = DB::select(
            'select A.*, U.* from amigos as A
            inner join usuarios as U on U.id = A.id_usuario
            where A.usuario_id = ?', [$id]);
            
            //Buscando dados dos posts e seus respectivos donos
            $post_amigos = DB::select(
                'select A.*, U.*, P.* from amigos as A
                inner join posts as P on P.usuario_id = A.id_usuario
                inner join usuarios as U on U.id = A.id_usuario
                where A.usuario_id = ?', [$id]);
                
            //Buscando Post do usuario logado e seus dados
            $post = DB::select(
                'select U.*, P.* from usuarios as U
                inner join posts as P on P.usuario_id = U.id                                
                where U.id = ?', [$id]);
                ...
                {       
                    ...
                    {   
                        //$url_img = public_path('ImgPosts').'\\'.$url_img;
                        //$img = Imagem::where('url_imagem', $url_img);
                        //$img->delete();
                        //unlink($url_img);
